...
bool FileUtil::DoesFileExist(std::string& filePath)
{
...
string FileUtil::GetExtension(std::string& filePath)
{
	if (filePath.size() < 1)
	{
...
std::string FileUtil::GetFileName(std::string& fileLocation)
{
	if (fileLocation.size() < 1)
	{
		throw invalid_argument("An empty file path was given");
...
string FileUtil::ReadFile(string fileLocation)
{
	try
	{
...
vector<string> FileUtil::ReadFileAsLines(string fileLocation)
{
