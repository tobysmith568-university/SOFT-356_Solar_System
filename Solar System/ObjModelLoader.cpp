...
ObjModelLoader::ObjModelLoader(FileUtil& _fileUtil, ConsoleUtil& _consoleUtil, MtlLoader& _mtlLoader)
	: fileUtil(_fileUtil), consoleUtil(_consoleUtil), mtlLoader(_mtlLoader)
{
}
...
void ObjModelLoader::GetModel(Model& model, std::string fileLocation, GLuint& program)
{
	...
	string folder = fileUtil.GetFolder(fileLocation);
	vector<string> fileLines = fileUtil.ReadFileAsLines(fileLocation);// Reads the file into memory
